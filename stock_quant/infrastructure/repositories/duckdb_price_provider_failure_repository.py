...
def is_permanent_failure(reason: str) -> bool:
    r = reason.lower()
    return any(k in r for k in PERMANENT_FAILURE_KEYWORDS)
...
class DuckDbPriceProviderFailureRepository:
    ...
    # ---------------------------------------------------------
    # READ — blacklist uniquement erreurs permanentes
    # ---------------------------------------------------------
    def get_recent_failures(
        ...
        with self._connect() as con:
            rows = con.execute("""
                SELECT provider_symbol, failure_reason
        ...
        return [
            ps for ps, reason in rows
            if ps and is_permanent_failure(reason)
        ]

    # ---------------------------------------------------------
    # WRITE — seulement erreurs permanentes
    # ---------------------------------------------------------
    def upsert_failures(
        self,
        provider_name: str,
        ...
        with self._connect() as con:
            for f in failures:
                if not is_permanent_failure(f["failure_reason"]):
                    continue
