...
from stock_quant.infrastructure.repositories.duckdb_universe_repository import (
    DuckDbUniverseRepository,
)
from stock_quant.pipelines.build_market_universe_pipeline import (
    BuildMarketUniversePipeline,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build market universe for stock-quant-oop."
    )
    parser.add_argument("--db-path", default=None, help="Path to DuckDB database file.")
    parser.add_argument(
        "--drop-existing",
        action="store_true",
        help="Rebuild schema before pipeline run.",
    )
    parser.add_argument(
        "--disallow-adr",
        action="store_true",
        help="Exclude ADR securities from the universe.",
    )
...
def main() -> int:
    ...
    with DuckDbUnitOfWork(session_factory) as uow:
        ...
        repository = DuckDbUniverseRepository(uow.connection)
