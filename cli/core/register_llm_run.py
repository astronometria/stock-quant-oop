...
from stock_quant.infrastructure.repositories.duckdb_llm_run_repository import (
    DuckDbLlmRunRepository,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Register an LLM enrichment run in llm_runs."
    )
    ...
    parser.add_argument(
        "--input-table",
        default="news_articles_normalized",
        help="Input table name.",
    )
    parser.add_argument(
        "--output-table",
        default="news_llm_enrichment",
...
def main() -> int:
    ...
    with DuckDbUnitOfWork(session_factory) as uow:
        ...
        repository = DuckDbLlmRunRepository(uow.connection)
