...
# =============================================================================
# Train aligned 20D LightGBM ranker
#
# Goal:
# - train on a clean 20-day forward return target
# - use canonical adjusted prices from price_bars_adjusted
# - reuse the existing train/val/test calendar from research_split_dataset
# - keep the implementation SQL-first and heavily commented
# =============================================================================
...
def load_training_frame(
    ...
    # -------------------------------------------------------------------------
    # We construct a 20-day forward target directly from adjusted prices.
    #
    # Important:
    # - target_return_20d is the true training target
    # - relevance_20d is a cross-sectional ranking target per date (0..4)
    # - dataset_split is inherited from the existing split calendar by date
    # -------------------------------------------------------------------------
    sql = f"""
    ...
    filtered AS (
        SELECT *
        FROM labeled
        WHERE dataset_split IN ('train', 'val', 'test')
          AND target_return_20d IS NOT NULL
    ),
    ranked AS (
        SELECT
            *,
...
def main() -> int:
    ...
    progress = tqdm(total=5, desc="train_lgbm_ranker_20d", leave=True)
    ...
    if df.empty:
        raise RuntimeError("Aligned 20D training frame is empty after filters.")
    ...
    metrics = {
        "trainer": "train_lgbm_ranker_20d_aligned.py",
        "db_path": str(db_path),
    ...
    print("===== TRAIN LGBM RANKER 20D ALIGNED =====")
    print(json.dumps({
        "model_path": str(model_path),
