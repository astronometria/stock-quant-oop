...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build conservative research universe.")
...
def main() -> int:
    ...
    if args.verbose:
        print(f"[build_research_universe] project_root={config.project_root}")
        print(f"[build_research_universe] db_path={config.db_path}")

    session_factory = DuckDbSessionFactory(config.db_path)
    with DuckDbUnitOfWork(session_factory) as uow:
        SchemaManager(uow).validate()
        ResearchUniverseSchemaManager(uow).initialize()
        pipeline = BuildResearchUniversePipeline(uow=uow)
        result = pipeline.run()
