...
from stock_quant.pipelines.build_sec_fact_normalized_pipeline import (
    BuildSecFactNormalizedPipeline,
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Normalize SEC XBRL facts into sec_fact_normalized."
    )
...
def main() -> int:
    ...
    if args.verbose:
        print(f"[build_sec_fact_normalized] project_root={config.project_root}")
        print(f"[build_sec_fact_normalized] db_path={config.db_path}")
    ...
    with DuckDbUnitOfWork(session_factory) as uow:
        ...
        pipeline = BuildSecFactNormalizedPipeline(repository=repository)
        result = pipeline.run()
