...
from stock_quant.pipelines.master_data_pipeline import BuildMasterDataPipeline


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build canonical company/instrument master tables.")
...
def main() -> int:
    ...
    if args.verbose:
        print(f"[build_master_data] project_root={config.project_root}")
        print(f"[build_master_data] db_path={config.db_path}")
    ...
    with DuckDbUnitOfWork(session_factory) as uow:
        ...
        repository = DuckDbMasterDataRepository(uow)
        pipeline = BuildMasterDataPipeline(repository=repository)
        result = pipeline.run()
